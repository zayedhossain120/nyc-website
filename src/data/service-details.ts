...
import { SAAS_CLOUD_DETAILS, SAAS_CLOUD_LOCAL_CONTENT } from "@/data/saas-cloud-services";
import { SERVICE_LOCAL_CONTENT, type NycContentBlock } from "@/data/service-local-content";
...
export interface ServiceDetail {
  ...
  pricing: PricingInfo;
  seo: SeoInfo;
  /** Keyword-bearing H1 (the short `title` remains the label). */
  h1: string;
  /** New York context block rendered on the page. */
  nyc: NycContentBlock;
  related?: string[];
  ...
  /** Additional long-form sections rendered after the scope list. */
  extraSections?: ExtraSection[];
}

export type BaseServiceDetail = Omit<ServiceDetail, "seo" | "h1" | "nyc">;

const BASE_SERVICE_DETAILS: BaseServiceDetail[] = [
  ...
  // ---------- Marketing & Growth ----------
  {
    ...
    successStories: {
      ...
      stories: [
        {
          type: "full",
          image: "/service/local-seo-nyc/google-business-profile-success.png",
        ...
        },
        {
          ...
          image: "/service/local-seo-nyc/Organic-Traffic-Growth.png",
        ...
        },
        {
          ...
          image: "/service/local-seo-nyc/Lead-Generation-Surge.png",
  ...
  },
  {
    ...
    related: ["local-seo-nyc", "technical-seo", "facebook-ads"],
    serviceAreas: {
...
/** Base services merged with their New York SEO and on-page content. */
export const SERVICE_DETAILS: ServiceDetail[] = BASE_SERVICE_DETAILS.map((service) => {
  const local = SAAS_CLOUD_LOCAL_CONTENT[service.slug] ?? SERVICE_LOCAL_CONTENT[service.slug];
  if (!local) {
    throw new Error(`Missing New York content for service "${service.slug}" in service-local-content.ts`);
  }
  return {
    ...service,
    h1: local.h1,
    nyc: local.nyc,
    faqs: service.faqs ?? local.faqs,
    seo: { title: local.seoTitle, description: local.seoDescription, keywords: local.keywords },
  };
});
