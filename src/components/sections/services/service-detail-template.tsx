...
export function getServiceDetailMetadata(service: ServiceDetail, path: string): Metadata {
  return {
    // Absolute: the SEO title already carries the brand, so skip the root template.
...
export function ServiceDetailTemplate({
  ...
  const schemas: object[] = [
    serviceSchema({
      ...
      offer: { price: parsePrice(service.pricing.amount), unit: unitLabel },
    }),
    breadcrumbSchema([
      { name: "Home", path: "/" },
  ...
  return (
    <main className="flex flex-1 flex-col">
      ...
      <section className="border-t border-subtle">
        <div className="mx-auto w-full max-w-7xl px-6 py-24 md:px-16">
          <Reveal className="overflow-hidden rounded-2xl border border-subtle bg-surface shadow-2xl shadow-black/40">
            ...
            <Image
              src={service.image}
              alt={`${service.title} for New York City businesses`}
              width={1200}
              height={900}
              priority
      ...
      <section className="border-t border-subtle">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-6 py-24 md:px-16">
          <SectionHeading eyebrow="New York" title={service.nyc.title} />
          <Reveal className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
            <p className="text-lg leading-relaxed text-secondary">{service.nyc.body}</p>
            <ul className="flex flex-col gap-3">
              {service.nyc.points.map((point) => (
                <li
                  key={point}
      ...
      {service.extraSections?.map((section) => (
        <section key={section.title} className="border-t border-subtle">
          <div className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-6 py-24 md:px-16">
            <SectionHeading eyebrow={section.eyebrow} title={section.title} />
            <Reveal className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
              <p className="text-lg leading-relaxed text-secondary">{section.body}</p>
              {section.points && (
                <ul className="flex flex-col gap-3">
